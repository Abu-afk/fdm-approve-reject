  ...
  useEffect(() => {
    const loadClaim = async () => {
      ...
      try {
        ...
        // FIRST: Check localStorage for updated claims
        const storedClaims = localStorage.getItem('fdm_claims');
        let claimData: ExpenseClaim | null = null;
        
        if (storedClaims) {
          const claims = JSON.parse(storedClaims);
          claimData = claims.find((c: ExpenseClaim) => c.claimId === claimId) || null;
        }
        
        if (claimData) {
          // Use data from localStorage
          setClaim(claimData);
          if (claimData.status === 'PAID') setProcessed(true);
        } else {
          // Fallback to API
